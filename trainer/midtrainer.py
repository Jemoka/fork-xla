...
class Midtrainer:
    ...
    def epoch(self):
        ...
        for indx in range(
            ...
            if (
                    ...
                    == (self.args.validation_interval) # so we don't ovelap with checkpoint
            ):
    ...
    def __dangerously_migrate_for_midtraining(self):
        if self.main_process():
            logger.info(f"TRAINER | Resetting myself for midtraining... This operation is **NOT IDEMPOTENT** and is in fact jank af.")
    ...
    def from_pretrained(cls, path, args, disable_wandb=True, distributed=False):
        ...
        new.__dangerously_migrate_for_midtraining()
